...
from caching import backends
from caching.compat import basestring
...
class CachedSession(OriginalSession):
    ...
    def __init__(self, cache_name='cache', backend=None, expire_after=None,
                 ...
                 filter_fn=lambda r: True, old_data_on_error=False, always_include_get_headers=[],
                 **backend_options):
        """
        ...
        self._always_include_get_headers = always_include_get_headers
    ...
    def send(self, request, **kwargs):
        ...
        def send_request_and_cache_response():
            ...
            if response.status_code in self._cache_allowable_codes:
                self.cache.save_response(cache_key, response, will_expire)
            response.from_cache = False
            return response
...
def install_cache(cache_name='cache', backend=None, expire_after=None,
                  allowable_codes=(200,), allowable_methods=('GET',),
                  filter_fn=lambda r: True, always_include_get_headers=[], session_factory=CachedSession,
                  **backend_options):
    """
    ...
    class _ConfiguredCachedSession(session_factory):
        def __init__(self):
            super(_ConfiguredCachedSession, self).__init__(
                ...
                always_include_get_headers=always_include_get_headers,
                filter_fn=filter_fn,
                **backend_options
            )
